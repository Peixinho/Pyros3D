...
static const AIToolDef kAITools[] = {
	...
	{ "canvas_drag",    "Move a UI element or resize it by one edge/corner, the same edit dragging its handle in canvas mode makes.",
		{ { "object", "string", "Object name", true },
		  ...
		  { "delta", "array", "[x, y] in canvas units", true } } },
	{ "apply_ui_style", "Apply a .uistyle to a UI element and record the link, so later edits to the style (or a palette swap) reach it on the next load. Styles carry look only - never text or layout.",
		{ { "object", "string", "Object name", true },
	...
	{ "set_ui",         "Set properties on an object's UI components. Rect: anchorMin/anchorMax/offsetMin/offsetMax/pivot ([x,y]). Image: tint/border ([r,g,b,a] / [l,t,r,b]), texture (path). Text: text, size, color, align, verticalAlign, wrap, sdf. Toggle: value, check, group. Slider: value, min, max, step, vertical, fill, handle. Input: text, placeholder, maxLength, password, readOnly, filter, blinkRate, onSubmit. List: items, selected, itemHeight, onSubmit. Dropdown: options, selected, placeholder. Menu item: submenu. Popup: open, modal, closeOnEscape, closeOnOutside, dialogElement. Any widget: interactable, onChange. Canvas: referenceWidth/referenceHeight, scaleMode, sortOrder. Button: interactable, transition, onClick, hoverTint/pressedTint/disabledTint, hoverTextColor/pressedTextColor/disabledTextColor, pressedOffset.",
		{ { "object", "string", "Object name", true },
		  { "properties", "object", "The properties to set; unknown keys are rejected", true } } },
	{ "add_ui",         "Add a screen-space UI component to an object: canvas, rect, image, text, button, toggle, slider, input, list, dropdown, menu or popup. Image, text and button add a rect if the object has none; a button also adds an image. The widget kinds also build the child elements they drive - a checkbox's tick, a slider's fill and handle, a list's rows.",
		{ { "object", "string", "Object name", true },
		  { "kind", "string", "canvas, rect, image, text, button, toggle, slider, input, list, dropdown, menu or popup", true },
	...
	{ "select",         "Select a game object in the editor (and focus it in the hierarchy). Empty name clears the selection.",
		{ { "name", "string", "Object name, or empty to deselect", false } } },
	{ "canvas_mode",    "Switch the viewport between the 3D scene and 2D canvas editing. Canvas mode hides the world, the floor grid and the axis widget, and shows the canvas bounds, a canvas-unit grid and the selected element's rect.",
	...
	{ "add_post_effect", "Add a post effect to the scene's chain. Order matters - each effect reads what the one before produced.",
		{ { "effect", "string", "Built-in name (see post_effects), e.g. Bloom, Vignette, SSAO, DepthOfField, MotionBlur", false },
		  ...
		  { "params", "object", "Parameter overrides {name: number or [numbers]}", false } } },
	{ "set_post_effect", "Turn one effect in the chain on/off and/or retune it. Parameters are merged, so setting one leaves the rest alone.",
		{ { "index", "integer", "Which entry (0-based) - the only way to tell two of the same effect apart", false },
		  { "effect", "string", "Built-in name, instead of an index", false },
		  { "asset", "string", "Effect asset path, instead of an index", false },
		  { "enabled", "boolean", "Run this effect", false },
		  { "params", "object", "Parameter overrides {name: number or [numbers]}", false } } },
	{ "remove_post_effect", "Remove one effect from the chain.",
		{ { "index", "integer", "Which entry (0-based)", false },
		  { "effect", "string", "Built-in name, instead of an index", false },
		  { "asset", "string", "Effect asset path, instead of an index", false } } },
	{ "move_post_effect", "Move an effect to another position in the chain - what changes is what it reads as its input.",
		{ { "index", "integer", "Which entry (0-based)", false },
		  { "effect", "string", "Built-in name, instead of an index", false },
		  { "asset", "string", "Effect asset path, instead of an index", false },
		  { "to", "integer", "Position to move it to (0-based)", true } } },
	{ "set_camera",     "Change an existing scene camera. Only the fields given are touched.",
		{ { "name", "string", "Camera object name", true },
		  { "projection", "string", "perspective or orthographic", false },
		  { "fov", "number", "Field of view in degrees (perspective only)", false },
		  { "size", "number", "Half-height of the view in world units (orthographic only)", false },
		  { "near", "number", "Near clip plane", false },
		  { "far", "number", "Far clip plane", false },
		  { "active", "boolean", "Make this the active camera", false } } },
