...
std::string TextEditor::GetIdentifierPrefixAtCursor() const
{
	std::string receiver, prefix;
	GetCompletionContext(receiver, prefix);
	return prefix;
}

void TextEditor::GetCompletionContext(std::string& outReceiver, std::string& outPrefix) const
{
	outReceiver.clear();
	outPrefix.clear();
	Coordinates at = GetActualCursorCoordinates();
	if (at.mLine < 0 || at.mLine >= (int)mLines.size())
		return;

	const Line& line = mLines[at.mLine];
	int i = GetCharacterIndex(at);
	if (i < 0) i = 0;
	if (i > (int)line.size()) i = (int)line.size();

	int start = i;
	while (start > 0)
	{
		unsigned char ch = (unsigned char)line[start - 1].mChar;
		if (!(isalnum(ch) || ch == '_'))
			break;
		--start;
	}

	outPrefix.reserve((size_t)(i - start));
	for (int j = start; j < i; ++j)
		outPrefix.push_back((char)line[j].mChar);

	if (start <= 0)
		return;
	const char sep = line[start - 1].mChar;
	if (sep != '.' && sep != ':')
		return;
	...
	while (rStart > 0)
	{
		unsigned char ch = (unsigned char)line[rStart - 1].mChar;
		if (!(isalnum(ch) || ch == '_'))
			break;
		--rStart;
	}
	if (rStart >= rEnd)
		return;
	outReceiver.reserve((size_t)(rEnd - rStart));
	for (int j = rStart; j < rEnd; ++j)
		outReceiver.push_back((char)line[j].mChar);
}
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			if (mState.mCursorPosition.mLine == lineNo)
			{
				...
				{
					float cxTrack = TextDistanceToLineStart(mState.mCursorPosition);
					mCursorScreenPos = ImVec2(textScreenPos.x + cxTrack, lineStartScreenPos.y);
					mCursorScreenPosValid = true;
				}
