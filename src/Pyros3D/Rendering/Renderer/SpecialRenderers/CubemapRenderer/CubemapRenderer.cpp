...
namespace p3d {
	...
	CubemapRenderer::CubemapRenderer(const uint32 Width, const uint32 Height) : IRenderer(Width, Height)
	{
		...
		environmentMap->CreateEmptyTexture(TextureType::CubemapNegative_X, TextureDataType::RGB8, Width, Height, false);
		environmentMap->CreateEmptyTexture(TextureType::CubemapNegative_Y, TextureDataType::RGB8, Width, Height, false);
		environmentMap->CreateEmptyTexture(TextureType::CubemapNegative_Z, TextureDataType::RGB8, Width, Height, false);
		environmentMap->CreateEmptyTexture(TextureType::CubemapPositive_X, TextureDataType::RGB8, Width, Height, false);
		environmentMap->CreateEmptyTexture(TextureType::CubemapPositive_Y, TextureDataType::RGB8, Width, Height, false);
		environmentMap->CreateEmptyTexture(TextureType::CubemapPositive_Z, TextureDataType::RGB8, Width, Height, false);
	...
	void CubemapRenderer::RenderCubeMap(SceneGraph* Scene, GameObject* AllSeeingEye, const f32 Near, const f32 Far)
	{
		...
		if (rmesh.size() > 0)
		{
			...
			if (lcomps.size() > 0)
			{
				for (std::vector<IComponent*>::iterator i = lcomps.begin(); i != lcomps.end(); i++)
				{
					...
					case LIGHT_TYPE::DIRECTIONAL:
					{
						...
						directionalLight.m[13] = (f32)type;	  	 directionalLight.m[14] = 0.f;  directionalLight.m[15] = -1.f;
					...
					case LIGHT_TYPE::POINT:
					{
						...
						pointLight.m[13] = (f32)type;	 pointLight.m[14] = 0.f; pointLight.m[15] = -1.f;
			...
			Matrix allSeeyingEyeTranslation; allSeeyingEyeTranslation.identity();
			allSeeyingEyeTranslation.Translate(AllSeeingEye->GetWorldTransformation().GetTranslation());
			...
			for (uint32 i = 0; i < 6; i++)
			{
				...
				ViewMatrix *= allSeeyingEyeTranslation.Inverse();
