...
namespace p3d {
	...
	DDGICompute::DDGICompute()
		: initialized(false),
		  ...
		  bRays(0), bIrr(0), bVis(0), bParams(0),
		...
		const std::string &shaderRoot)
	{
		...
		bParams = dev.CreateStorageBuffer(6 * 4 * (uint32)sizeof(f32), 8, NULL);
		...
		const uint32 probeBudget)
	{
		...
		while (done < wanted)
		{
			...
			const DeviceHandle bufs[9] = { bTris,bNodes,bIdx,bMats,bLights,bRays,bIrr,bVis,bParams };

			// Trace.
			dev.BindComputePipeline(0, tracePipeline);
			for (uint32 b = 0; b < 9; b++) dev.BindStorageBuffer(0, bufs[b], b);
	...
	void DDGICompute::Shutdown()
	{
		...
		IRenderDevice &dev = GetActiveRenderDevice();
		const DeviceHandle bufs[9] = { bTris,bNodes,bIdx,bMats,bLights,bRays,bIrr,bVis,bParams };
		for (uint32 i = 0; i < 9; i++) if (bufs[i] != 0) dev.DestroyStorageBuffer(bufs[i]);
		bTris=bNodes=bIdx=bMats=bLights=bRays=bIrr=bVis=bParams=0;

		const DeviceHandle pipes[3] = { tracePipeline, irrPipeline, visPipeline };
		const DeviceHandle progs[3] = { traceProgram, irrProgram, visProgram };
		const DeviceHandle stages[3] = { traceStage, irrStage, visStage };
		...
		tracePipeline=irrPipeline=visPipeline=0;
		traceProgram=irrProgram=visProgram=0;
		traceStage=irrStage=visStage=0;
		initialized = false;
		cursor = 0;
	}
