...
namespace p3d {
...
#ifdef _WIN32
		// Windows has no HOME; LOCALAPPDATA is the equivalent of ~/.cache
		// for machine-local, non-roaming derived data.
		const char *appdata = getenv("LOCALAPPDATA");
		if (appdata && appdata[0])
			return std::string(appdata) + "/pyros3d";
#endif
	...
	// std::filesystem rather than mkdir(2): MSVC's <sys/stat.h> has neither
	// the two-argument mkdir nor S_ISDIR.
	static bool EnsureDirectoryExists(const std::string &path)
	{
		...
		// create_directories reports false+no-error when another thread won
		// the race, so re-test rather than trusting its return.
