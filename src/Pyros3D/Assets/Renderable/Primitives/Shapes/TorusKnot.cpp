...
namespace p3d {
	...
	TorusKnot::TorusKnot(const f32 radius, const f32 tube, const uint32 segmentsW, const uint32 segmentsH, const f32 p, const f32 q, const uint32 heightscale, bool smooth, bool flip, bool TangentBitangent)
	{
		isFlipped = flip;
		isSmooth = smooth;
	...
	Vec3 TorusKnot::GetPos(const f32 u, const f32 v) const
	{
		f32 cu = cos(u);
		f32 su = sin(u);
