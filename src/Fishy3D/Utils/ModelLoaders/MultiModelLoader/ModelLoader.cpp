...
namespace Fishy3D {
    ...
    void ModelLoader::Load(const std::string& Filename)
    {
        ...
        if (!assimp_model)
        {
            ...
            for (unsigned int i=0;i<assimp_model->mNumMeshes;++i)
            {
                // loop through meshes
                ...
                if (mesh->HasBones())
                {
                    ...
                    for ( unsigned int k = 0; k < mesh->mNumBones; k++)
                    {
                        ...
                        short int boneID = GetBoneID(mesh->mBones[k]->mName.data);
                    ...
                    for (unsigned int j = 0;j< mesh->mNumVertices; j++)
                    {
                        ...
                        std::vector<float>weightValue(4,0.f);
            ...
            for (unsigned int i=0;i<assimp_model->mNumMaterials;++i)
            {
    ...
    void ModelLoader::GetBone(aiNode* bone, const short int &parentID)
    {
    ...
    unsigned ModelLoader::GetBoneID(const std::string& BoneName)
    {
        StringID ID (MakeStringID(BoneName));
        return skeleton[ID].self;
    }
    unsigned ModelLoader::GetBoneID(const StringID& BoneID)
    {        
    ...
    void ModelLoader::GetBoneChilds(std::map<StringID,Bone> Skeleton, const int& id, const unsigned int& iterations)
    {
        for (std::map<StringID, Bone>::iterator i=Skeleton.begin();i!=Skeleton.end();i++)
        {            
            if ((*i).second.parent==id) 
            {
                for (unsigned j=0;j<iterations+1;j++) if (j==iterations) std::cout << " |_"; else std::cout << "   ";
