    ...
    void AnimationLoader::Load(const std::string& Filename)
    {
        ...
        if (!assimp_model)
        {
            ...
            if (assimp_model->HasAnimations()==true) 
            {
                ...
                for (unsigned int i=0;i<assimp_model->mNumAnimations;i++)
                {
                    Animation animation;
                    ...
                    for (unsigned int k=0;k<assimp_model->mAnimations[i]->mNumChannels;k++)
                    {
                        Channel channel;
                        ...
                        for (unsigned int l=0;l<assimp_model->mAnimations[i]->mChannels[k]->mNumPositionKeys;l++)
                        {
                            vec3 Pos;
                        ...
                        for (unsigned int l=0;l<assimp_model->mAnimations[i]->mChannels[k]->mNumRotationKeys;l++)
                        {
                            Quaternion Rot;
                        ...
                        for (unsigned int l=0;l<assimp_model->mAnimations[i]->mChannels[k]->mNumScalingKeys;l++)
                        {
                            vec3 Scale;
